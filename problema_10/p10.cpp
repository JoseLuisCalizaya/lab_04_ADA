...
int UGRAPHmaxDegree(Graph G) {
    int max_degree = 0;
    for (vertex v = 0; v < G->V; ++v) {
        int current_degree = GRAPHoutdeg(G, v);
...
int main() {
  ...
  */
  ...
  // Ejercicio 10: Calcular grado máximo (lo tratamos como no dirigido para el ejemplo)
  std::cout << "\n--- (E10) Grado maximo ---" << std::endl;
  std::cout << "El grado maximo es: " << UGRAPHmaxDegree(G) << std::endl;
